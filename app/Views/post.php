    ...
    if (result.isConfirmed) {
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                // Tampilkan pesan kesalahan kepada pengguna atau tangani kesalahan sesuai kebutuhan
                Swal.fire(
                    'Error!',
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response.error == false) {
              // Bersihkan isi elemen sebelum menambahkan data baru
              $("#show_posts").empty();

              // Iterasi melalui data dan tambahkan ke elemen
              $.each(response.message, function(index, post) {
                // Buat elemen baru untuk setiap posting
                var postElement = $("<div>").addClass("post").html("<strong>" + post.title + "</strong><p>" + post.body + "</p>");
                ...
                // Tambahkan elemen ke div dengan id 'show_posts'
                $("#show_posts").append(data);

              });
            } else {
              // Tampilkan pesan kesalahan jika diperlukan
              alert("Failed to fetch posts. Error: " + response.message);
            }
          }
        });
      }
